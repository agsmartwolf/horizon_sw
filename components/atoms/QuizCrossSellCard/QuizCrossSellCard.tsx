...
const QuizCrossSellCard: React.FC<QuizCrossSellCardProps> = ({
  ...
  return (
    <article className={['lg:max-w-[556px]', props.className ?? ''].join(' ')}>
      <form
        className="grid h-full grid-cols-[89px_auto] grid-rows-[89px_63px_56px] gap-x-4 gap-y-6 rounded-2xl bg-black p-6 shadow-3xl lg:grid-cols-[200px_auto] lg:grid-rows-[auto_68px_auto_56px] lg:gap-x-6 lg:gap-y-0"
        ...
        <div className="hidden text-sm font-semibold tracking-wide text-black lg:col-[2/3] lg:my-[19px] lg:block">
          <Link href={href} className="flex h-fit items-center gap-x-2">
            <span>{hrefCta}</span>
            <ArrowRight className="w-[13.33px]" />
          </Link>
        </div>
