...
const propsCallback: GetStaticProps<StaticPageProps> = async context => {
  ...
  const termsConditionsPage = contentPages?.results?.find(page => {
    if (page && page.slug === 'terms-conditions') {
      return page;
    }
  });

  if (!termsConditionsPage?.published) {
    return {
      notFound: true,
    };
  }
  ...
  const editorData = (await fetchPageData(
    termsConditionsPage.slug as string,
...
const TermsConditionsPage: NextPage<ServerSideProps<typeof getStaticProps>> = ({
  title,
  sections,
}) => {
