...
const propsCallback: GetStaticProps<StaticPageProps> = async context => {
  ...
  const privacyPolicyPage = contentPages?.results?.find(page => {
    if (page && page.slug === 'privacy-policy') {
      return page;
    }
  });

  if (!privacyPolicyPage?.published) {
    return {
      notFound: true,
    };
  }
  ...
  const editorData = (await fetchPageData(
    privacyPolicyPage.slug as string,
...
const PrivacyPolicyPage: NextPage<ServerSideProps<typeof getStaticProps>> = ({
  title,
  sections,
}) => {
